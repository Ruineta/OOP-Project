...
public class CartScreenController {
    ...
    void placeOrderPressed(ActionEvent event) {
        Alert alert = new Alert(Alert.AlertType.INFORMATION, cart.placeOrder_hacd());
    ...
    void btnPlayPressed(ActionEvent event) {
        ...
        Alert alert;
        try {
            alert = new Alert(Alert.AlertType.NONE, media.playGUI_hacd());
    ...
    void btnRemovePressed(ActionEvent event) {
        Media media = tblMedia.getSelectionModel().getSelectedItem();
        cart.removeMedia_hacd(media);
        costLabel.setText(cart.totalCost_hacd() + " $");
    }
            ...
            private void showFilteredMedia(String keyword) {
                ...
                if (!keyword.isEmpty() && radioBtnFilterId.isSelected()) {
                    filteredList.setPredicate(media -> {
                        String idString = String.valueOf(media.getId_hacd());
                        return idString.equals(keyword);
                    });
                } else if (!keyword.isEmpty() &&  radioBtnFilterTitle.isSelected()) {
                    filteredList.setPredicate(media -> {
                        String title = media.getTitle_hacd().toLowerCase();
