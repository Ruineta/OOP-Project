...
public class Cart {
    ...
    public String addMedia_hacd(Media media) throws LimitExceededException {
        if (itemsOrdered.size() >= MAX_NUMBERS_ORDERED) {
            throw new LimitExceededException("ERROR: The number of media has reached its limit");
        } else if (itemsOrdered.contains(media)) {
            return(media.getTitle_hacd() + " is already in the cart!");
        } else {
            qtyOrdered +=1;
            itemsOrdered.add(media);
            return (media.getTitle_hacd() + " has been added!");
        }
    }
    public void removeMedia_hacd(Media media) {
        if (itemsOrdered.size() == 0) {
            ...
            if (itemsOrdered.remove(media)) {
                System.out.println(media.getTitle_hacd() + " has been removed from the cart.");
                qtyOrdered -=1;
            } else {
    ...
    // Searcb to remove
    public Media searchToRemove_hacd(String title) {
    ...
    public void searchByTitle_hacd(String keyword) {
        boolean matchFound = false;
        for (Media media : itemsOrdered) {
            if (media.isMatch_hacd(keyword)) {
    ...
    public void searchByCategory_hacd(String category) {
        boolean found = false;
        for (Media media : itemsOrdered) {
            if (media.getCategory_hacd().equalsIgnoreCase(category)) {
                System.out.println("Found " + media);
                found = true;
            }
        }
    ...
    public void searchByPrice_hacd(float maxCost) {
        boolean matchFound = false;
        for (Media media : itemsOrdered) {
            if (media.getCost_hacd() <= maxCost) {
                System.out.println("Found " + media);
                matchFound = true;
            }
        }
    ...
    public void searchByPrice_hacd(float minCost, float maxCost) {
        boolean matchFound = false;
        for (Media media : itemsOrdered) {
            if (media.getCost_hacd() >= minCost && media.getCost_hacd() <= maxCost) {
                System.out.println("Found " + media);
                matchFound = true;
            }
        }
    ...
    public void searchByID_hacd(int id) {
        boolean found = false;
        for (Media media : itemsOrdered) {
            if (media.getId_hacd() == id) {
                System.out.println("Found " + media);
                found = true;
            }
        }
    ...
    public float totalCost_hacd() {
        float totalCost = 0;
        for (Media media : itemsOrdered) {
            totalCost += media.getCost_hacd();
        }
    ...
    public void print_hacd() {
        ...
        System.out.println("Total cost: " + totalCost_hacd());
        System.out.println("***************************************************");
    }

    public void empty_hacd() {
        if (itemsOrdered.size() == 0) {
            System.out.println("Nothing to remove!");
        } else {
            qtyOrdered = 0;
    ...
    // Sort media in cart
    public void sortMediaByTitle_hacd() {
    ...
    public void sortMediaByCost_hacd() {
